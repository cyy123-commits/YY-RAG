...
def run_query_graph(query: str, session_id: str, is_stream: bool):
    ...
    # 清空上一次查询的进度状态，防止第二次 query 时 done_list 残留
    ...
    # 初始状态设为 processing，避免前端显示 unknown
    update_task_status(session_id, "processing", push_queue=is_stream)
...
async def query(request: QueryRequest,background_tasks: BackgroundTasks):
    ...
    if is_stream:
        # 只要开启流式处理，业务中就是将数据，插入到队列中！ {session_id , queue [update_task_state , add_running_task,add_done_list]}
